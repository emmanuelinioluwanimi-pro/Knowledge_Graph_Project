...
def get_kg() -> nx.DiGraph:
    """
    Load the multimodal Knowledge Graph.

    Supports two formats (checked in this order):
      1. Per-manual JSON files: mmkg_full__<manual>_.json — each with
         {"nodes": [...], "edges": [...]}. All files merged into one graph.
         This is the current format Inioluwa produces.
      2. Legacy single GraphML: mmkg.graphml — fallback for older exports.
    """
    global _kg
    if _kg is not None:
        return _kg

    json_files = sorted(KG_DIR.glob("mmkg_full__*.json"))
    graphml_files = sorted(KG_DIR.glob("*.graphml"))

    if json_files:
        print(f"[init] loading KG from {len(json_files)} JSON files")
        g = nx.DiGraph()
        for jf in json_files:
            # Derive manual tag from filename: mmkg_full__Filter_drier_shell_.json
            #   -> "Filter_drier_shell"
            manual_tag = jf.stem.replace("mmkg_full__", "").strip("_")
            with open(jf) as fh:
                data = json.load(fh)
            ...
            for node in data.get("nodes", []):
                ...
                # Stamp the source manual onto every node — useful for
                # per-manual analysis in evaluation.
                attrs.setdefault("manual", manual_tag)
            ...
            for edge in data.get("edges", []):
                ...
                # Normalise: the retrieval code (Section 12) reads either
                # 'relation' or 'label'. This schema uses 'type'.
                if "type" in attrs and "relation" not in attrs:
                    attrs["relation"] = attrs["type"]
                g.add_edge(src, tgt, **attrs)
            print(f"       {jf.name}: +{g.number_of_nodes() - n_before} nodes, "
                  f"+{g.number_of_edges() - e_before} edges  "
                  f"(manual={manual_tag})")
        _kg = g
        print(f"[init] KG total: {_kg.number_of_nodes()} nodes, "
              f"{_kg.number_of_edges()} edges")
        return _kg

    if graphml_files:
        path = graphml_files[0]
        print(f"[init] loading KG from legacy GraphML: {path}")
        _kg = nx.read_graphml(path)
        print(f"[init] KG: {_kg.number_of_nodes()} nodes, "
              f"{_kg.number_of_edges()} edges")
        return _kg

    print(f"[warn] no KG files in {KG_DIR}. KG traversal disabled.")
    _kg = nx.DiGraph()
    return _kg
