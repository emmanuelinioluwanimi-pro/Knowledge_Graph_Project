...
# =============================================================================
# 12. KG TRAVERSAL  --  matched to Inioluwa's schema (entity_name, alias, relation)
# =============================================================================
#
# NOTE FOR THE KG RETRIEVAL WORKSTREAM
# ------------------------------------
# The functions below (_query_candidates, _find_seed_nodes, _node_label,
# kg_retrieve) are the KG-retrieval API. If you are the person improving KG
# retrieval quality (e.g. adding manual-scoped seed matching, better entity
# extraction, weighted BFS, etc.), modify these four functions in place.
# The signature of `kg_retrieve(query, hops, mode, vector_chunks)` is fixed
# because retrieve() and evaluate.py depend on it — everything downstream
# passes through this function. `mode` and `vector_chunks` are already plumbed
# through so a scoping strategy that uses either or both can be dropped in
# without touching anything else.
#
def _query_candidates(query: str) -> List[str]:
    """
    Extract candidate strings from the query for KG matching:
      - Part-number-ish tokens (contain a digit)
      - Multi-word capitalized phrases
      - Individual words length >= 4
    """
    cands = set()
    # Part number pattern: alphanumeric with digit anywhere
    for m in re.findall(r"[A-Za-z0-9][\w\-\./]*\d[\w\-\./]*", query):
        cands.add(m)
    # Whole tokens
    for w in re.findall(r"[A-Za-z][A-Za-z\-]{3,}", query):
        cands.add(w.lower())
    # Bigrams (small model of "cover bolts", "core shaft", etc.)
    words = re.findall(r"[A-Za-z\-]+", query)
    for i in range(len(words) - 1):
        cands.add(f"{words[i]} {words[i+1]}".lower())
    return sorted(cands, key=len, reverse=True)


def _find_seed_nodes(g: nx.DiGraph, candidates: List[str], cap: int = 12) -> List[str]:
    """
    Match candidate strings against entity_name / alias / part_number.
    Simple substring match; returns the first `cap` unique matches.
    (KG-retrieval improvement workstream: replace this with weighted / scoped
    seed selection as needed. Callers only depend on the return type.)
    """
    if g.number_of_nodes() == 0 or not candidates:
        return []
    seeds: List[str] = []
    seen = set()
    ...
    for node, attrs in g.nodes(data=True):
        haystack = " ".join(
            str(attrs.get(k, "")) for k in ("entity_name", "alias", "part_number")
        ).lower()
...
def _node_label(g: nx.DiGraph, node: str) -> str:
    ...
    name = a.get("entity_name", "") or node
    return f"{name} [{et}]" if et else name
...
def kg_retrieve(query: str,
    ...
    """
    BFS from matched seeds, return typed triples as strings.
    ...
    mode : "kg_only" | "hybrid"
        Retrieval configuration for the evaluation ablation. Baseline mode
        never calls this function.
        - "kg_only" : this function's output IS the answer context. Should not
          depend on vector search.
        - "hybrid"  : this function's output is used alongside vector chunks.
          `vector_chunks` are available here as an optional signal.
    vector_chunks : List[Dict] | None
        Reranked vector-search results (only populated in hybrid mode). Passed
        through so a scoping strategy can use them if desired. Currently ignored.

    KG-IMPROVEMENT NOTE
    -------------------
    Current implementation is intentionally minimal: extract candidate strings
    from the query, match them lexically against entity_name / alias /
    part_number, BFS out `hops` steps, dedupe. No manual scoping, no
    entity-type weighting. The KG-retrieval workstream is expected to replace
    this body with better scoping / weighting logic while preserving the
    function signature.
    """
    g = get_kg()
    ...
    cands = _query_candidates(query)
    seeds = _find_seed_nodes(g, cands)
    if not seeds:
        return []
    ...
    for _ in range(hops):
        ...
        for node in frontier:
            for _, tgt, d in g.out_edges(node, data=True):
                rel = d.get("relation") or "RELATED_TO"
                triples.append(
                    f"({_node_label(g, node)}) --[{rel}]--> ({_node_label(g, tgt)})"
                )
                if tgt not in visited:
                    nxt.add(tgt); visited.add(tgt)
            for src, _, d in g.in_edges(node, data=True):
                rel = d.get("relation") or "RELATED_TO"
                triples.append(
                    f"({_node_label(g, src)}) --[{rel}]--> ({_node_label(g, node)})"
                )
                if src not in visited:
                    nxt.add(src); visited.add(src)
        frontier = nxt
        if not frontier:
            break
